...
`cat /etc/lsb-release` или `lsb_release -a` - показать версию Ubuntu
...
> ****Note**** :
> systemctl работает через раз в WSL 2, а если работает то иногда, выдает ошибку
>
> System has not been booted with systemd as init system (PID 1). Can't operate.
>
> Заменить на `sudo service start имя_службы`
...
<details>
<summary>Базовые пакеты:</summary>
...
`htop` – наше все, могучий монитор ресурсов, отображающий всю информацию о текущем потреблении ресурсов системы.
Позволяющий также отправлять системные вызовы процессам, менять их приоритет в системе и многое другое.

`telnet` – на практике, часто используем для проверки доступности удаленных портов. В случае если порт закрыт, мы будем
точно знать, что проблема не на нашей стороне.
...
`strace` – утилита для анализа системных вызовов процесса, используется для расширенного анализа работающих в системе
процессов. По факту мы используем strace в тех случаях, когда логи сервисов не указывают на конкретную проблему.
...
- `-F`, `--form` — curl эмулирует заполненную форму, с нажатием кнопки отправки.
- `-o`, `--output` — сохранить эти данные в локальный файл, используя параметры, минуя stdout
- `-D`, `--dump-header` — Сохраняет в указанный дамп-файл полученные заголовки
- `-u`,  `--user user:password` — Логин и пароль для аутентификации на сервере.

`curl https://jsonplaceholder.typicode.com/posts` - GET запрашивает у сервера определенный ресурс.

`curl -X POST -d "userId=5&title=Hello World&body=Post body." https://jsonplaceholder.typicode.com/posts` - POST
используется для создания ресурса на сервере. Если ресурс существует, он перезаписывается.

`curl -X POST -H "Content-Type: application/json" -d '{"userId": 5, "body": "Post body."}' https://jsonplaceholder.typicode.com/posts`

- Чтобы отправить данные в формате JSON

`curl -X DELETE https://jsonplaceholder.typicode.com/posts/5` - DELETE удаляет указанный ресурс с сервера.
...
-D result.headers \ // прописываем вывод полученных заголовков в файл дампа
-o result.json // указание вывести результат запроса в файл
```
...
<details>
<summary>Примеры использования curl PHP</summary>

[Примеры использования cURL в PHP](https://snipp.ru/php/curl)
...
curl_setopt($ch, CURLOPT_URL, "https://api.weather.yandex.ru/v2/forecast?". http_build_query($val));
// Установка CURLOPT_RETURNTRANSFER (вернуть ответ в виде строки)
// пользовательские заголовки в HTTP-запрос
curl_setopt($ch, CURLOPT_HTTPHEADER, [
    'X-Apple-Tz: 0',
    'X-Apple-Store-Front: 143444,12'
]);

//  Устанавливает несколько параметров для сеанса cURL + дополнительные опции которые модно прописать 
curl_setopt_array($ch,[...] + (array($option)))
...
$f = file_get_contents("https://api.weather.yandex.ru/v2/forecast/?lat=55.721560&lon=84.929553&lang=ru_RU",false,$context);
```
...
[Справочник языка](https://php.ru/manual/)
...
`php.ini-development` - для разработчика (для среды разработки)

`php.ini-production` - для пользователя ( настройки, подходящие для производственной среды) Стоит использовать так как
имеет оптимизированные настройки по умолчанию для повышения производительности и безопасности.

`php/7.4/snapshot.txt` - возможности ядро и список подключаемых модулей
...
`php-fpm` - увидим на каком pid процесс работает

`php -h` - помощь (почаще пользоваться)

- `-a` - перейдете к интерпретатору PHP

- `-m` - список установленных модулей

- `-c <путь>|<файл>` - Найдите файл php.ini в этом каталоге

- `-t` - устанавливает указанную директорию «directory root», используеться в связке с -s (в ней потом можно будет
  перемещаться через url)

`php -S localhost:8000 -c php.ini` - Чтобы задать параметры конфигурации для конкретного проекта, просто добавьте файл
php.ini в свой проект, а затем запустите встроенный сервер. Это особенно полезно для настроек, которые не могут быть
установлены во время выполнения (ini_set())

`php -r 'print_r(get_defined_constants());'` - Передайте код PHP для выполнения непосредственно в командной строк

`php -S [localhost:8080] или (http://localhost:8080)` запуск веб сервера ( Ctrl-C остановить )
...
- `master` - значение, скомпилированное в PHP, либо заданное с помощью основной php.ini директивы. То есть значение,
  которое действует, когда запускается PHP, прежде чем он выполнит любой ваш код.
- `local`- это значение, которое действует на момент вызова phpinfo(). Это локальное значение является конечным
  результатом любых переопределений

`php -c ./php.ini index.php` - запустить php с настроенным или другим файлом php.ini

`php -c ./php.ini -S localhost:8080` -для запуска сервиса с другим php.ini (php -S localhost:8080 index.php -c ./php.ini - так настройки не войдут в силу)

`sudo service php8.0-fpm status` - включение php8.0-fpm сервера (используется для сокетов)
...
`$x("//img")` - получить XPath (в консоли)
...
`Alt` + `Insert` - создает конструктор общего кода (get, set, construct)
...
`https://github.dev/` - войти github IDE
